    ...
    for attempt in range(retries):
        try:
            ...
            if r.status_code in (429,500,502,503,504):
                wait=min(30,2**attempt); print(f'YouTube {r.status_code}; retrying in {wait}s'); time.sleep(wait); continue
...
def parse_iso(s):
    ...
    return int(m.group(1) or 0)*3600+int(m.group(3) or 0)+int(m.group(2) or 0)*60 if m else 0
...
def main():
    ...
    for i in range(0,len(candidates),50):
        for x in api('videos',{'part':'snippet,contentDetails,status,statistics','id':','.join(v for v,_ in candidates[i:i+50])}).get('items',[]):
            ...
            if cid not in verified or x.get('status',{}).get('privacyStatus')!='public' or not x.get('status',{}).get('embeddable'): continue
            if BAD.search(title) or dur<2700 or not GOOD.search(title) or not DUB.search(text): continue
            langs=[l for l,terms in LANGS.items() if any(re.search(r'\b'+re.escape(t)+r'\b',text,re.I) for t in terms)]
            if not langs: continue
            thumbs=sn.get('thumbnails',{}); thumb=(thumbs.get('maxres') or thumbs.get('standard') or thumbs.get('high') or thumbs.get('medium') or thumbs.get('default') or {}).get('url'); st=x.get('statistics',{}); views=int(st.get('viewCount',0)); likes=int(st.get('likeCount',0)); channel=sn.get('channelTitle') or channel_by_id[cid].get('name','Unknown')
            items.append({'id':x['id'],'title':title,'slug':slug(title),'youtubeVideoId':x['id'],'youtubeUrl':f'https://www.youtube.com/watch?v={x["id"]}','thumbnail':thumb,'language':langs[0],'audioLanguage':'Hindi','dubbed':True,'creator':{'name':channel,'channelId':cid,'channelUrl':f'https://www.youtube.com/channel/{cid}'},'description':desc,'metadata':{'year':year_from(text),'genre':[],'durationSeconds':dur},'statistics':{'views':views,'likes':likes},'publishedAt':sn.get('publishedAt'),'verification':{'status':'verified','method':'official-channel-whitelist'},'algorithm':{'trendingScore':0,'recommendationScore':0}})
    clusters=[]
    for m in items:
    ...
    print(f'Catalog: {len(out)} Hindi-dubbed South movies from {len(official)} verified channels')
